...
function genFunctionContent_LL(wxmlName, config={}, source, funcName) {
  // 起始
  let content = `${config.funcName}` + "=function(_,_v,_n,_p,_s,_wp,_wl,$gwn,$gwl,$gwh,wh,$gstack,$gwrt,gra,grb,TestTest,wfor,_ca,_da,_r,_rz,_o,_oz,_1,_1z,_2,_2z,_m,_mz,nv_getDate,nv_getRegExp,nv_console,nv_parseInt,nv_parseFloat,nv_isNaN,nv_isFinite,nv_decodeURI,nv_decodeURIComponent,nv_encodeURI,nv_encodeURIComponent,$gdc,nv_JSON,_af,_gv,_ai,_grp,_gd,_gapi,$ixc,_ic,_w,_ev,_tsd){return function(path,global){\u000aif(typeof global==='undefined'){if (typeof __GWX_GLOBAL__==='undefined')global={};else global=__GWX_GLOBAL__;}if(typeof __WXML_GLOBAL__ === 'undefined') {__WXML_GLOBAL__={};\u000a}__WXML_GLOBAL__.modules = __WXML_GLOBAL__.modules || {};\u000a"
  // 
  ...
  content = content.replace(new RegExp(`\\${funcName}_${config.num+1}`, 'g'), `${config.funcName}_1`)
  // console.log(content)
  return content
}
function genCommonContent_LL(source, funcName){
...
function wxmlToJS(options={}) {
  ...
  for(let file of options.files){
    if (typeof options.replaceContent[file] === 'string') {
      // 写入替换内容
      nfs.mkdirSync(path.dirname(path.resolve(tmp, file)), {recursive:true})
      nfs.writeFileSync(path.resolve(tmp, file), options.replaceContent[file])
    }else{
      // 复制原文件
      nfs.mkdirSync(path.dirname(path.resolve(tmp, file)), {recursive:true})
      nfs.copyFileSync(path.resolve(options.cwd, file), path.resolve(tmp, file))
    }
  }
  // 使用临时目录执行wcc
  options.cwd = tmp
    ...
    if(options.lazyload){
      ...
      const resultObj = {
          ...
          generateFunctionContent:{
              __COMMON__: genCommonContent_LL(str, options.genfuncname)
          },
      }
      // 获取Linux wcc的输出数据，用于分析
      // process.stdout.write(str)
      // process.exit(1)
      const indexArr = eval(str.match(/var x=(\[.*\]);/)[1])
      const pageConfig = {}
      indexArr.forEach((ele, i) => {
      ...
      for(let key in pageConfig){
          resultObj.generateFunctionName[key] = pageConfig[key].funcName
          resultObj.generateFunctionContent[key] = genFunctionContent_LL(key, pageConfig[key], str, options.genfuncname)
      }
